...
def test_organizations():
    ...
    res = requests.post(f"{BASE_URL}/admin/organizations", json=payload)
...
def test_users(org_id):
    ...
    # 2. List
    res = requests.get(f"{BASE_URL}/admin/users?org_id={org_id}")
    if res.status_code == 200:
        users = res.json()
...
def test_audit_logs(org_id):
    ...
    res = requests.post(f"{BASE_URL}/admin/audit-logs?org_id={org_id}", json=payload)
    if res.status_code == 200:
    ...
    # 2. Verify Chain
    res = requests.get(f"{BASE_URL}/admin/audit-logs/verify?org_id={org_id}")
    if res.status_code == 200:
        data = res.json()
        if data.get("valid"):
...
def test_messages(org_id):
    ...
    res = requests.get(f"{BASE_URL}/messages?org_id={org_id}&limit=1")
    hits = res.json().get("hits", [])
    
    if not hits:
        log_info("No messages in new org. Falling back to Org 13 for metadata tests.")
        res = requests.get(f"{BASE_URL}/messages?org_id=13&limit=1")
        hits = res.json().get("hits", [])
        active_org = 13
    else:
        active_org = org_id
    ...
    # 1. Detail
    res = requests.get(f"{BASE_URL}/messages/{msg_id}?org_id={active_org}")
    if res.status_code == 200:
    ...
    # 2. PII Scan
    res = requests.get(f"{BASE_URL}/messages/{msg_id}/pii-scan?org_id={active_org}")
    if res.status_code == 200:
        data = res.json()
    ...
    # 3. Redaction Preview
    res = requests.get(f"{BASE_URL}/messages/{msg_id}/preview-redacted?org_id={active_org}")
    if res.status_code == 200:
        data = res.json()
    ...
    # 4. Headers
    res = requests.get(f"{BASE_URL}/messages/{msg_id}/headers?org_id={active_org}")
    if res.status_code == 200:
...
def test_legal_holds(org_id):
    ...
    res = requests.post(f"{BASE_URL}/admin/holds?org_id={org_id}", json=payload)
    if res.status_code == 200:
...
def test_cases(org_id, msg_id):
    ...
    res = requests.post(f"{BASE_URL}/cases?org_id={org_id}", json=payload)
    if res.status_code == 200:
    ...
    res = requests.post(f"{BASE_URL}/cases/{case_id}/items?org_id={org_id}", json=payload)
    if res.status_code == 200:
    ...
    res = requests.post(f"{BASE_URL}/cases/{case_id}/export?org_id={org_id}", json=payload)
    if res.status_code == 200:
...
def test_analytics(org_id):
    ...
    res = requests.get(f"{BASE_URL}/admin/analytics?org_id={org_id}")
    if res.status_code == 200:
        data = res.json()
...
def main():
    print(f"{Colors.HEADER}{Colors.BOLD}=== OpenArchive API Tester ==={Colors.ENDC}\n")
